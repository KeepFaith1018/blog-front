...
function defineComponent(options, extraOptions) {
  return isFunction(options) ? (
    // #8236: extend call and config.name access are considered side-effects
...
function resolveAsset(type, name, warnMissing = true, maybeSelfReference = false) {
  ...
  if (instance) {
    ...
    const res = (
      // local registration
      // check instance[type] first which is resolved for config API
...
function createComponentInstance(vnode, parent, suspense) {
  ...
  const instance = {
    ...
    components: null,
    directives: null,
    // resolved props and emits config
